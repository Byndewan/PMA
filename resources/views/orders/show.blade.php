...
<div class="space-y-4">
    ...
    <div class="bg-white rounded-lg border border-gray-100 p-4 shadow-xs">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <p class="mt-1 text-gray-800">{{ $order->customer_phone }}</p>
            </div>
            @endif
            <div>
    ...
    <div class="bg-white rounded-lg border border-gray-100 p-4 shadow-xs">
        <h3 class="text-lg font-semibold mb-3 text-gray-800">Update Status</h3>
        <form action="{{ route('orders.update', $order->id) }}" method="POST">
            @csrf
            <div class="flex flex-col sm:flex-row gap-3">
                <select name="status" class="flex-1 border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition">
                ...
                <button type="submit" class="inline-flex items-center justify-center px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm">
                    Update Status
                </button>
            </div>
        </form>
